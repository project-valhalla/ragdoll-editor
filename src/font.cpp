...
void newfont(char *name, char *tex, int *defaultw, int *defaulth, int *offsetx, int *offsety, int *offsetw, int *offseth)
{
    ...
    f->tex = loadtexture(tex, 3, &f->scalex, &f->scaley);
    f->chars.setsize(0);
    f->defaultw = *defaultw;
    f->defaulth = *defaulth;
    f->offsetx = *offsetx;
...
static int draw_char(int c, int x, int y)
{
    ...
    float tc_left    = (info.x + curfont->offsetx) / float(curfont->scalex);
    float tc_top     = (info.y + curfont->offsety) / float(curfont->scaley);
    float tc_right   = (info.x + info.w + curfont->offsetw) / float(curfont->scalex);
    float tc_bottom  = (info.y + info.h + curfont->offseth) / float(curfont->scaley);
...
struct textcolor
{
    ...
    textcolor() {}
    textcolor(uchar x, uchar y, uchar z) : x(x), y(y), z(z) {}
};

//stack[sp] is current color index
static void text_color(char c, char *stack, int size, int &sp, textcolor color, int a) 
{
    if(c=='s') // save color
    {   
        c = stack[sp];
    ...
    else
    {
        ...
        switch(c)
        {
            case '0': color = textcolor( 64, 255, 128); break;   // green: player talk
            case '1': color = textcolor( 96, 160, 255); break;   // blue: "echo" command
            case '2': color = textcolor(255, 192,  64); break;   // yellow: gameplay messages 
            case '3': color = textcolor(255,  64,  64); break;   // red: important errors
            case '4': color = textcolor(128, 128, 128); break;   // gray
            case '5': color = textcolor(192,  64, 192); break;   // magenta
            case '6': color = textcolor(255, 128,   0); break;   // orange
