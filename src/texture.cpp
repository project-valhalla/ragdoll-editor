...
void resizetexture(int w, int h, bool mipmap, GLenum target, int &tw, int &th)
{
    ...
    w = min(w, sizelimit);
    h = min(h, sizelimit);
    if(mipmap || (target!=GL_TEXTURE_RECTANGLE_ARB && (w&(w-1) || h&(h-1))))
    {
        tw = th = 1;
        while(tw < w) tw *= 2;
        while(th < h) th *= 2;
...
void createtexture(int tnum, int w, int h, void *pixels, int clamp, bool mipmap, GLenum format = GL_RGB, GLenum target = GL_TEXTURE_2D, int pw = 0, int ph = 0)
{
    ...
    uploadtexture(target, format, tw, th, format, GL_UNSIGNED_BYTE, pixels, pw, ph, mipmap && pixels);
}
       
GLuint loadtexture(const char *name, int clamp, int *xs, int *ys, int *bpp)
{
    int w, h, b;
    uchar *data = loadtga(path(name, true), w, h, b); 
    if(!data) { conoutf(CON_ERROR, "failed loading %s", name); return 0; }
    GLenum format = texformat(b*8);
    if(!format) { conoutf(CON_ERROR, "failed loading %s", name); delete[] data; return 0; }
    ...
    GLuint tex;
    glGenTextures(1, &tex);
    createtexture(tex, w, h, data, clamp, true, format);
